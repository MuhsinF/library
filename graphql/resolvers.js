...
export const resolvers = {
 ...
 Mutation: {
  addBook: async (_, args, context) => {
   const {title, author, date, collection} = args
   const {userId} = context

   return prisma.book.create({data: {title, author, date: new Date(date), collection, userId}})
  },
  modifyBook: async (_, args, context) => {
   const {bookId, title, author, date, collection, rating} = args
   const {userId} = context
   ...
   return prisma.book.update({
    ...
    data: {title, author, date: new Date(date), collection, rating}
   })
  },
  signup: async (_, args) => {
